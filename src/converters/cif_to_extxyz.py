...
def convert_cif_to_extxyz(cif_file, output_path, append=False):
    """
    Convert a single CIF file to EXTXYZ format (can be manipulated easier by chemiscope).
    
    Args:
        cif_file (str): Path to input CIF file.
        output_path (str): Path for output EXTXYZ file.
    ...
    try:
        ...
        ase.io.write(output_path, images=atoms, format='extxyz', append=append)
        
        print(f"Converted: {cif_file} -> {output_path}")
        return True
